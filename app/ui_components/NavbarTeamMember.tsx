...
export default function NavbarTeamMember({ userName }: { userName?: string }) {
        ...
        <header className="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 flex items-center justify-between px-8 sticky top-0 z-30">
            ...
            <div className="flex items-center gap-6">
                <button className="relative text-slate-500 hover:text-emerald-600 transition-colors">
                    ...
                    <span className="absolute top-0 right-0 w-2 h-2 bg-rose-500 rounded-full border border-white dark:border-slate-900" />
                </button>

                <div className="flex items-center gap-3 border-l border-slate-200 dark:border-slate-700 pl-6">
                    <div className="text-right hidden md:block">
                        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 leading-none">{userName || "Team Member"}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Contributor</p>
                    </div>
                    <Link href="/team_member/profile" title="My Profile">
                        <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-500 flex items-center justify-center text-white font-bold shadow-md shadow-emerald-500/20 ring-2 ring-white dark:ring-slate-800 cursor-pointer hover:opacity-90 transition-opacity uppercase">
                            {initials}
                        </div>
                    </Link>
                </div>
