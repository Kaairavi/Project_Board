...
import { LayoutDashboard, FolderKanban, ListTodo, LogOut, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function SidebarManager({ isCollapsed, setIsCollapsed, userName }: SidebarProps) {
    const pathname = usePathname();
    const isDashboard = pathname === "/manager/dashboard" || pathname === "/manager";
    const displayName = userName
        ? (userName === "Manager_User" ? "Manager" : userName.replace(/_/g, " "))
        : "Manager";
        ...
        >
            ...
            <div className={`flex items-center gap-3 mb-8 overflow-hidden ${isCollapsed ? "justify-center px-0" : "px-2"}`}>
                ...
                {!isCollapsed && (
                    ...
                    >
                        Project<span className="text-indigo-600">Board</span> <span className="text-xs font-normal text-slate-400 align-top">Mgr</span>
                    </motion.span>
                )}
            </div>
            ...
            <nav className="space-y-2 flex-1">
                ...
                <NavItem
                    ...
                    label="My Projects"
                    active={pathname.startsWith("/manager/projects")}
                    icon={FolderKanban}
                    isCollapsed={isCollapsed}
                />
                <NavItem
                    href="/manager/tasks"
                    label="All Tasks"
                    active={pathname.startsWith("/manager/tasks")}
                    icon={ListTodo}
                    isCollapsed={isCollapsed}
                />
            </nav>
